...
void copyRE(Record &e1,Record *&e2) {
          ...
          e1.totalNumber= e2->totalNumber;
		   exchar(e1.name,e2->name);
		   exchar(e1.author,e2->author);	 
}
...
Status insertBTree(BTree &t,int k,Record &e){
     ...
     if(r.tag==1) {          //已经存在 
     	 q = r.pt;
		 q->recptr[r.i]->totalNumber +=e.totalNumber;
         q->recptr[r.i]->reNumber +=e.totalNumber;
		 return ERROR;
	 }
      ...
      while(TRUE){
        ...
        switch(n){        
         ...
         case 2:  
		     printf("请输入要清除库存的书号:");             
             scanf("%d",&key);
             if(OK==deleteBTree(t,key )){
                printf("删除成功\n");	
			 } else{
			 	printf("书库中不存在该书，删除失败\n");
			 }
             printf("=================\n");
             printBTree(t,lev);
             printf("=================\n");
